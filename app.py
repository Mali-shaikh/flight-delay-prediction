...
# --- Sidebar Inputs ---
with st.sidebar:
    st.image(ROOT / "header_banner.png", use_container_width=True)
    ...
    with st.expander("✈️ Flight Logistics", expanded=True):
        carrier_name = st.selectbox("Airline", list(CARRIERS.keys()), index=0)
        origin_name = st.selectbox("Origin Airport", list(AIRPORTS.keys()), index=6)
        dest_name = st.selectbox("Destination Airport", list(AIRPORTS.keys()), index=7)
        distance = st.number_input("Distance (miles)", 50, 5000, 2475)
        crs_elapsed_time = st.number_input("Duration (min)", 30, 600, 330)

    st.info("💡 Adjust the parameters above to see the prediction update in real-time.")
...
with col_main:
    ...
    st.image(ROOT / "header_banner.png", use_container_width=True, caption="Advanced Aviation Analytics")
    ...
    try:
        ...
        # Display Results in a sexy way
        res_col1, res_col2 = st.columns([0.4, 0.6])
        
        with res_col1:
            st.markdown("### 🎯 Result")
            if pred == 1:
                st.markdown('<div class="prediction-text" style="color: #FF4B2B;">⚠️ LIKELY DELAYED</div>', unsafe_allow_html=True)
                st.markdown(f"The model predicts a **{(prob*100):.1f}%** chance of delay (15+ min).")
            else:
                st.markdown('<div class="prediction-text" style="color: #28a745;">✅ ON TIME</div>', unsafe_allow_html=True)
                st.markdown(f"The model is **{(100-prob*100):.1f}%** confident this flight will be on time.")
            
            st.warning("**Recommendation:** Check for live updates 3 hours before departure.")
        ...
        with res_col2:
            fig = go.Figure(go.Indicator(
                ...
                value = prob * 100,
                domain = {'x': [0, 1], 'y': [0, 1]},
                title = {'text': "Delay Risk Index", 'font': {'size': 24, 'color': 'white'}},
                gauge = {
...
with bottom_col1:
    st.markdown('<div class="metric-card">🌍 <b>Origin</b><br>' + origin_name + '</div>', unsafe_allow_html=True)
with bottom_col2:
    st.markdown('<div class="metric-card">🏁 <b>Destination</b><br>' + dest_name + '</div>', unsafe_allow_html=True)
with bottom_col3:
    st.markdown('<div class="metric-card">✈️ <b>Airline</b><br>' + carrier_name + '</div>', unsafe_allow_html=True)

st.caption("FlightWise AI v1.2 | MSc Data Science Project")
